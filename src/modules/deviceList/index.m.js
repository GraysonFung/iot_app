...
export default modelExtend(listModel, {
	...
	effects: {
		*deviceList({ payload }, { call, put, select }) {  // eslint-disable-line
			const Loaded = yield select(state => state.deviceList.Loaded)
			if (Loaded) {
				console.log("拦截请求")
				return
			}
			...
			const dataSource = yield select(state => state.deviceList.dataSource)
			const current = yield select(state => state.deviceList.pagination.current)
			const pageSize = yield select(state => state.deviceList.pagination.pageSize)


			const dataBlobs = yield select(state => state.deviceList.dataBlobs)
			const sectionIDs = yield select(state => state.deviceList.sectionIDs)
			const rowIDs = yield select(state => state.deviceList.rowIDs)
